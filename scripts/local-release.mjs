...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `${CLI_NAME}-${platform}.zip` : `${CLI_NAME}-${platform}.tar.gz`;
...
function createCliShim(installDirectory) {
	...
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, `${CLI_NAME}.cmd`))) {
			writeFileSync(join(installDirectory, `${CLI_NAME}.cmd`), `@ECHO off\r\n"%~dp0node_modules\\.bin\\${CLI_NAME}.cmd" %*\r\n`);
			writeFileSync(join(installDirectory, `${CLI_NAME}.ps1`), `& "$PSScriptRoot/node_modules/.bin/${CLI_NAME}.ps1" @args\n`);
			return;
		}
		writeFileSync(join(installDirectory, `${CLI_NAME}.cmd`), `@ECHO off\r\n"%~dp0node_modules\\.bin\\${CLI_NAME}.exe" %*\r\n`);
		writeFileSync(join(installDirectory, `${CLI_NAME}.ps1`), `& "$PSScriptRoot/node_modules/.bin/${CLI_NAME}.exe" @args\n`);
		return;
	}
	symlinkSync(join("node_modules", ".bin", CLI_NAME), join(installDirectory, CLI_NAME));
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `${CLI_NAME}-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? `${CLI_NAME}.exe` : CLI_NAME)} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? `${CLI_NAME}.cmd` : CLI_NAME)} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? `${CLI_NAME}.cmd` : CLI_NAME)} --help`);
	}
}
