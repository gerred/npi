...
**Source:** [`@gerred/npi-tui`](https://github.com/earendil-works/pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@gerred/npi-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `NPI_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@gerred/npi-tui";
...
Import from `@gerred/npi-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@gerred/npi-tui";
```
...
```typescript
import { matchesKey, Key } from "@gerred/npi-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@gerred/npi-tui";
...
```typescript
import { getMarkdownTheme } from "@gerred/npi-coding-agent";
import { Markdown } from "@gerred/npi-tui";
...
For letting users pick from a list of options. Use `SelectList` from `@gerred/npi-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@gerred/npi-coding-agent";
import { DynamicBorder } from "@gerred/npi-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@gerred/npi-tui";
...
```typescript
import { BorderedLoader } from "@gerred/npi-coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@gerred/npi-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@gerred/npi-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@gerred/npi-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@gerred/npi-coding-agent";
import { matchesKey, truncateToWidth } from "@gerred/npi-tui";
