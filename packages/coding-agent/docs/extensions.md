...
| Package | Purpose |
|---------|---------|
| `@gerred/npi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@gerred/npi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@gerred/npi-tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@gerred/npi-coding-agent";
...
```typescript
import { isBashToolResult } from "@gerred/npi-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@gerred/npi-coding-agent";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@gerred/npi-coding-agent";
...
```typescript
import { SessionManager } from "@gerred/npi-coding-agent";
...
```typescript
import type { AutocompleteItem } from "@gerred/npi-tui";
...
```typescript
import { withFileMutationQueue } from "@gerred/npi-coding-agent";
...
import { Type } from "typebox";
import { StringEnum } from "@gerred/npi-ai";
import { Text } from "@gerred/npi-tui";
...
**Important:** Use `StringEnum` from `@gerred/npi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@gerred/npi-coding-agent";
...
```typescript
import { createBashTool } from "@gerred/npi-coding-agent";
...
```typescript
import { keyHint } from "@gerred/npi-coding-agent";
...
```typescript
import { Text, Component } from "@gerred/npi-tui";
...
import { matchesKey } from "@gerred/npi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@gerred/npi-coding-agent";
