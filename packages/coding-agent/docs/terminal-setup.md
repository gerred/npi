...
On WSL, WezTerm may require a visible hardware cursor for IME candidate window positioning. If CJK IME candidates do not follow the text cursor, set `NPI_HARDWARE_CURSOR=1` before running npi or set `showHardwareCursor` to `true` in settings.
...
If you want the hardware cursor visible, set `NPI_HARDWARE_CURSOR=1` before running npi (disabled by default for compatibility).
