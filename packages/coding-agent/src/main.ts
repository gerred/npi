...
import { type ImageContent, modelsAreEqual } from "@gerred/npi-ai";
...
export async function main(args: string[], options?: MainOptions) {
	resetTimings();
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.NPI_OFFLINE);
	if (offlineMode) {
		process.env.NPI_OFFLINE = "1";
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.NPI_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: NPI_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
