...
export function getSelfUpdateUnavailableInstruction(
	...
	if (method === "bun-binary") {
		return `Download from: https://github.com/gerred/npi/releases/latest`;
	}
	const command = getSelfUpdateCommandForMethod(method, packageName, updatePackageName, npmCommand);
	if (command) {
...
export function getPackageDir(): string {
	...
	const envDir = process.env.NPI_PACKAGE_DIR;
	if (envDir) {
...
export const PACKAGE_NAME: string = pkg.name || "@gerred/npi-coding-agent";
export const APP_NAME: string = piConfigName || "npi";
export const APP_TITLE: string = piConfigName ? APP_NAME : "π";
export const CONFIG_DIR_NAME: string = pkg.piConfig?.configDir || ".npi";
export const VERSION: string = pkg.version || "0.0.0";

// e.g., NPI_CODING_AGENT_DIR
...
const DEFAULT_SHARE_VIEWER_URL = "https://api.noumena.com/session/";

/** Get the share viewer URL for a gist ID */
export function getShareViewerUrl(gistId: string): string {
	const baseUrl = process.env.NPI_SHARE_VIEWER_URL || DEFAULT_SHARE_VIEWER_URL;
