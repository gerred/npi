...
export type KnownProvider = "noumena";
...
export interface OpenAICompletionsCompat {
	...
	/** Format for reasoning/thinking parameters. npi only supports Noumena's Kimi reasoning fields. */
	thinkingFormat?: "noumena";
	/** Whether the provider supports the `strict` field in tool definitions. Default: true. */
	supportsStrictMode?: boolean;
	cacheControlFormat?: never;
	/** Whether to send known session-affinity headers (`session_id`, `x-client-request-id`, `x-session-affinity`) from `options.sessionId` when caching is enabled. Default: false. */
	sendSessionAffinityHeaders?: boolean;
	/** Whether the provider supports long prompt cache retention. Default: false. */
