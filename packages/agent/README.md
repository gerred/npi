# @gerred/npi-agent-core

Stateful agent with tool execution and event streaming. Built on `@gerred/npi-ai`.

## Installation

```bash
npm install @gerred/npi-agent-core
```
...
```typescript
declare module "@gerred/npi-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@gerred/npi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@gerred/npi-agent-core";
